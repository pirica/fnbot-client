...
exports.run = async (FM, args, respond, lng, FN) => {
    ...
    try {
        ...
        if (isFriended) return await respond(FM.friend.id, i18next.t("addfriend.already_friended", {
            ns: "bot"
        }));
        await FN.client.inviteFriend(Profile.id);
        return await respond(FM.friend.id, i18next.t("addfriend.success", {
            ns: "bot",
            lng,
            username: `[${Profile.displayName}]`
        }));
    } catch (err) {
        return await respond(FM.friend.id, i18next.t("addfriend.error", {
            ns: "bot",
            lng
        }));
    };

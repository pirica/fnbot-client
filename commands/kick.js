...
exports.run = async (FM, args, respond, lng, FN) => {
  ...
  try {
    ...
    if (!Member) {
      return await respond(FM.friend.id, i18next.t('kick.not_in_party', { ns: 'bot', lng }))
    };
    await FN.fortnite.party.kick(Profile.id)
    return await respond(FM.friend.id, i18next.t('kick.success', { ns: 'bot', lng, username: `[${Profile.displayName}]` }))
  } catch (err) {
    return await respond(FM.friend.id, i18next.t('kick.error', { ns: 'bot', lng }))
  };
}

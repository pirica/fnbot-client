...
exports.run = async (FM, args, respond, lng, FN) => {
    ...
    try {
        ...
        if (!isFriended) return await respond(FM.friend.id, i18next.t("rmvfriend.not_friended", {
            ns: "bot"
        }));
        await FN.client.removeFriend(Profile.id);
        return await respond(FM.friend.id, i18next.t("rmvfriend.success", {
            ns: "bot",
            lng,
            username: `[${Profile.displayName}]`
        }));
    } catch (err) {
        return await respond(FM.friend.id, i18next.t("rmvfriend.error", {
            ns: "bot",
            lng
        }));
    };

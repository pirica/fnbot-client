...
fs.readdir(path.join(__dirname, './commands/'), (err, files) => {
    files.forEach(file => {
        var f = require(path.join(__dirname, `./commands/${file}`));
        if (!f.run) return;
            ...
            if (FN instanceof String) {
                ...
                FN.client.communicator.on("friend:message", async (FM) => {
                    ...
                    if (commands[command.toLowerCase()]) {
                        let cmd = commands[command.toLowerCase()];
                        if (cmd.groupOnly && !FN.fortnite.party.members.filter(m => m.id == FM.friend.id)[0]) return await FN.fortnite.communicator.sendMessage(FM.friend.id, i18next.t("msg_notingroup", {
                            ns: "bot",
                            lng: config.preferred_language
                        }));
                        if (cmd.ownerOnly && FM.friend.id !== FN.client.owner.id) return await FN.fortnite.communicator.sendMessage(FM.friend.id, i18next.t("msg_owneronly", {
                            ns: "bot",
                            lng: config.preferred_language
                        }));
                ...
                setInterval(async () => {
                    ...
                    let build = await server.checkBuild();
                    if (!build.server_off && JSON.stringify(build) != JSON.stringify(config.build)) {
                        config.build = build;
                        ...
                        require("fs").writeFileSync(process.cwd() + "/config.json", JSON.stringify(config));
                    };
                }, 1000 * 60);
            };
        });

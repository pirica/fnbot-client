        ...
        return new Promise(function (resolve, reject) {
            ...
            client.init().then(async (success) => {
                ...
                if (options.bot.login.token_2fa) {
                    console.log("[2] " + i18next.t("logging_in.2fa", {
                        ns: "console",
                        lng: options.preferred_language
                    }));
                    login = await client.login({
                        email: options.bot.login.email,
                        password: options.bot.login.password,
                        twoFactorCode: await Authenticator.GetAuthToken(options.bot.login.token_2fa)
                    });
                } else {
                    console.log("[2] " + i18next.t("logging_in.no_2fa", {
                        ns: "console",
                        lng: options.preferred_language
                    }));
                    login = await client.login({
